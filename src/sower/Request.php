...
class Request
{
    ...
    public function subDomain(): string
    {
        if (is_null($this->subDomain)) {
            ...
            if ($rootDomain) {
                // 配置域名根 例如 Sower.org.cn 如果是国家级域名 com.cn net.cn 之类的域名需要配置
    ...
    public function param($name = '', $default = null, $filter = '')
    {
        if (empty($this->mergeParam)) {
            ...
            // 当前请求参数和URL地址中的参数合并
            $this->param = array_merge($this->param, $this->get(false), $vars, $this->routes(false));
     ...
     * @return mixed
     */
    public function routes($name = '', $default = null, $filter = '')
    {
        if (is_array($name)) {
            return $this->only($name, $this->routes, $filter);
        }

        return $this->input($this->routes, $name, $default, $filter);
    }
        ...
        switch (strtolower($type)) {
            ...
            case 's':
                if (is_scalar($data)) {
                    ...
                    throw new \InvalidArgumentException('Array not defined：' . gettype($data));
                }
                break;
        }
    }
    ...
    {
        if (!in_array($type, ['param', 'get', 'post', 'put', 'patch', 'routes', 'delete', 'cookie', 'session', 'env', 'request', 'server', 'header', 'file'])) {
            return false;
        }
