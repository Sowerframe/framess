...
if (!function_exists('SoLan')) {
    //获取语言变量值
    function SoLan($name, $vars = [], $lang = '')
    {
        ...
        return \sowers\Lang::get($name, $vars, $lang);
    }
 }
...
if (!function_exists('format_bytes')) {
 ...
 if (!function_exists('datetime')) {
  ...
  if (!function_exists('human_date')) {
    ...
    {
        return \sums\Date::human($time, $local);
    }
 }
  ...
  if (!function_exists('cdnurl')) {
    ...
    function cdnurl($url, $domain = false)
    {
        ...
        $url = preg_match($regex, $url) ? $url : \sowers\Config::get('upload.cdnurl') . $url;
    ...
    function addtion($items, $fields)
    {
        ...
        foreach ($fieldsArr as $k => $v) {
            if ($v['model']) {
                ...
                $model = $v['name'] ? \sowers\Db::name($v['name']) : \sowers\Db::table($v['table']);
            }
...
if (!function_exists('download')) {
    /**
     * 获取\sower\response\Download对象实例
     * @param  string $filename 要下载的文件
     * @param  string $name     显示文件名
     * @param  bool   $content  是否为内容
     * @param  int    $expire   有效期（秒）
     * @return \sower\response\File
     */
...
if (!function_exists('input')) {
    ...
    function input(string $key = '', $default = null, $filter = '')
    {
        ...
        if ($pos = strpos($key, '.')) {
            ...
            if (in_array($method, ['get', 'post', 'put', 'patch', 'delete', 'routes', 'param', 'request', 'session', 'cookie', 'server', 'env', 'path', 'file'])) {
...
if (!function_exists('Request_input')) {
     ...
     * @return mixed
     */
    function Request_input(string $key = '', $default = null, $filter = '')
    {
        if (0 === strpos($key, '?')) {
            $key = substr($key, 1);
            $has = true;
...
if (!function_exists('json')) {
    /**
     * 获取\sower\response\Json对象实例
     * @param  mixed $data    返回的数据
     * @param  int   $code    状态码
     * @param  array $header  头部
     * @param  array $options 参数
     * @return \sower\response\Json
     */
...
if (!function_exists('jsonp')) {
    /**
     * 获取\sower\response\Jsonp对象实例
     * @param  mixed $data    返回的数据
     * @param  int   $code    状态码
     * @param  array $header  头部
     * @param  array $options 参数
     * @return \sower\response\Jsonp
     */
...
if (!function_exists('redirect')) {
    /**
     * 获取\sower\response\Redirect对象实例
     * @param  mixed         $url    重定向地址 支持Url::build方法的地址
     * @param  array|integer $params 额外参数
     * @param  int           $code   状态码
     * @return \sower\response\Redirect
     */
...
if (!function_exists('trace')) {
     ...
     * @return array|void
     */
    function trace($log = '[sower]', string $level = 'log')
    {
        if ('[sower]' === $log) {
            return Log::getLog();
        }
...
if (!function_exists('xml')) {
    /**
     * 获取\sower\response\Xml对象实例
     * @param  mixed $data    返回的数据
     * @param  int   $code    状态码
     * @param  array $header  头部
     * @param  array $options 参数
     * @return \sower\response\Xml
     */
