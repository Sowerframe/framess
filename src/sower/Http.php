...
class Http
{
    ...
    protected function runWithRequest(Request $request)
    {
        ...
        return $this->app->routes->dispatch($request, $withRoute);
    }
    ...
    protected function loadRoutes(): void
    {
        ...
        if ($this->app->routes->config('route_annotation')) {
            // 自动生成注解路由定义
            if ($this->app->isDebug()) {
                $this->app->console->call('routes:build', [$this->name]);
            }
    ...
    {
        return $this->app->getRootPath() . 'routes' . DIRECTORY_SEPARATOR . ($this->isMulti() ? $this->getName() . DIRECTORY_SEPARATOR : '');
    }
    ...
    {
        if ($this->app->config->get('app.apps_switch', true)) {
            // 自动多应用识别
