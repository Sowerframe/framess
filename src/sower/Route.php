...
use sower\exception\RouteNotFoundException;
use sower\routes\Dispatch;
use sower\routes\dispatch\Url as UrlDispatch;
use sower\routes\Domain;
use sower\routes\Resource;
use sower\routes\Rule;
use sower\routes\RuleGroup;
use sower\routes\RuleItem;
use sower\routes\RuleName;
use sower\routes\Url as UrlBuild;
class Route
{
    /**
     * REST定义
    ...
    {
        $this->app      = $app;
        $this->config   = array_merge($this->config, $app->config->get('routes'));
        ...
        if (is_file($app->getRuntimePath() . 'routes.php')) {
            // 读取路由映射文件
            $this->import(include $app->getRuntimePath() . 'routes.php');
        }
     ...
     * @return RuleItem
     */
    public function rule(string $rule, $routes = null, string $method = '*'): RuleItem
    {
        return $this->group->addRule($rule, $routes, $method);
    }
    /**
     * @access public
     * @param string $rule  路由规则
     ...
     * @return RuleItem
     */
    public function match(string $rule, $routes): RuleItem
    {
        return $this->rule($rule, $routes, 'GET');
    }
    /**
     * 设置跨域有效路由规则
     * @access public
     ...
     * @return RuleGroup
     */
    public function group($name, $routes = null): RuleGroup
    {
        ...
        return (new RuleGroup($this, $this->group, $name, $routes))
            ->lazy($this->lazy)
            ->mergeRuleRegex($this->mergeRuleRegex);
    }
     ...
     * @return RuleItem
     */
    public function any(string $rule, $routes): RuleItem
    {
        return $this->rule($rule, $routes, '*');
    }
     ...
     * @return RuleItem
     */
    public function get(string $rule, $routes): RuleItem
    {
        return $this->rule($rule, $routes, 'GET');
    }
     ...
     * @return RuleItem
     */
    public function post(string $rule, $routes): RuleItem
    {
        return $this->rule($rule, $routes, 'POST');
    }
     ...
     * @return RuleItem
     */
    public function put(string $rule, $routes): RuleItem
    {
        return $this->rule($rule, $routes, 'PUT');
    }
     ...
     * @return RuleItem
     */
    public function delete(string $rule, $routes): RuleItem
    {
        return $this->rule($rule, $routes, 'DELETE');
    }
     ...
     * @return RuleItem
     */
    public function patch(string $rule, $routes): RuleItem
    {
        return $this->rule($rule, $routes, 'PATCH');
    }
     ...
     * @return Resource
     */
    public function resource(string $rule, string $routes): Resource
    {
        return (new Resource($this, $this->group, $rule, $routes, $this->rest))
            ->lazy($this->lazy);
    }
     ...
     * @return RuleItem
     */
    public function redirect(string $rule, string $routes = '', int $status = 301): RuleItem
    {
        return $this->rule($rule, $routes, '*')->redirect()->status($status);
    }
    ...
    {
        return $this->group->miss($routes, $method);
    }
